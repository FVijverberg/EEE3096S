...
#define F_SIGNAL 300 // Frequency of output analog signal - check what cutoff frequency of filter is
/* USER CODE END PD */
...
void EXTI0_1_IRQHandler(void)
{
	...
	if (HAL_GetTick() >= debounceTime)
		{
			__HAL_TIM_DISABLE_DMA(&htim2, TIM_DMA_CC1);
			HAL_DMA_Abort_IT(&hdma_tim2_ch1);

			LUT_counter++;
			debounceTime = HAL_GetTick() + 500;

			switch(LUT_counter%3)
			{
				case 0 :
					HAL_DMA_Start_IT(&hdma_tim2_ch1, Sin_LUT, DestAddress, NS );
				    delay(3000);
